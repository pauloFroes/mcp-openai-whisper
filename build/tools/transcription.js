...
async function splitAndTranscribe(audioPath, language, includeTimestamps, tempDir) {
    const { stdout: durationOut } = await execCommand("ffprobe", ["-v", "quiet", "-show_entries", "format=duration", "-of", "csv=p=0", audioPath], { timeout: 10_000 });
...
export function registerTranscriptionTools(server) {
    server.registerTool("transcribe_audio", {
        ...
        description: "Transcribe a local audio file using OpenAI Whisper API. Supports any audio format (mp3, wav, m4a, etc.). Handles files larger than 25MB by automatically splitting into chunks. Optimized for Portuguese (PT-BR) but supports any language.",
        inputSchema: {
            file_path: z
                .string()
        ...
        try {
            ...
            return toolError(`Failed to transcribe audio: ${error.message}`);
        }
        finally {
